...
def backtest_weekly():
    ...
    predicted_for = latest_file.replace("weekly_predictions_", "").replace(".csv", "")

    print(f"[INFO] Backtesting weekly predictions for: {predicted_for}")
    pred_df = pd.read_csv(os.path.join(LOG_DIR, latest_file))
    ...
    actual_next_week = actual_df[actual_df["week"] == next_week_str]
    actual_close_map = actual_next_week.set_index("symbol")["close_price"].to_dict()
