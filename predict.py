...
def predict():
    ...
    try:
        predicted_date = (
            datetime.strptime(latest_date, "%d%m%Y") + timedelta(days=1)
        ).strftime("%d%m%Y")
    except ValueError:
        predicted_date = "Next Day"

    pred_df = processed_df[
        (processed_df["prediction"] == 1) &
        (processed_df["date"] == latest_date) &
        (processed_df["confidence"] >= CONFIDENCE_THRESHOLD)
    ].copy()
